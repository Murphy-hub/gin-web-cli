# [GIN-Admin](https://github.com/Murphy-hub/gin-admin) efficiency assistant

> A gin-web efficiency assistant that provides project initialization, code generation, greatly improves work efficiency, and quickly completes the development of business logic.
...
```bash
go install github.com/Murphy-hub/gin-web-cli@latest
```

### Create a new project

```bash
gin-web-cli new -d ~/go/src --name testapp --desc 'A test API service based on golang.' --pkg 'github.com/xxx/testapp'
```

### Quick generate a struct

```bash
gin-web-cli gen -d ~/go/src/testapp -m SYS --structs Dictionary --structs-comment "Dictionaries management" --structs-router-prefix
```
...
```bash
./gin-web-cli gen -d ~/go/src/testapp -m SYS -c dictionary.yaml
```
...
```bash
./gin-web-cli gen -d ~/go/src/testapp -m SYS -c parameter.yaml
```

### Remove a struct from the module

```bash
gin-web-cli rm -d ~/go/src/testapp -m SYS -s Dictionary
```
...
```text
NAME:
   gin-web-cli new - Create a new project

USAGE:
   gin-web-cli new [command options] [arguments...]
...
OPTIONS:
   ...
   --git-url value        Use git repository to initialize the project (default: https://github.com/Murphy-hub/gin-admin.git)
...
```text
NAME:
   gin-web-cli generate - Generate structs to the specified module, support config file

USAGE:
   gin-web-cli generate [command options] [arguments...]
...
```text
NAME:
   gin-web-cli remove - Remove structs from the module

USAGE:
   gin-web-cli remove [command options] [arguments...]
